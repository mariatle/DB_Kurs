...
class DashboardHomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["incident_open"] = Incident.objects.filter(
                                  status__in=["open", "investigation"]).count()

        # последние критические тревоги (10 шт.)
        ctx["latest_alarms"] = (
            Alarm.objects
...
# карта устройств — оставляем как было
from django.views.generic import TemplateView as _TV
class DeviceMapView(_TV):
